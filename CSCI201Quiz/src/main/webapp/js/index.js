...
$(document).ready(function () {
    ...
    searchButton.addEventListener('click', function () {
        ...
        let categorySelect = document.getElementsByClassName("category-select")[0];
    ...
    function getCategoryData() {
        let ajaxUrl = 'http://localhost:8082/CSCI201Quiz/category';
        console.log(ajaxUrl);
        $.ajax({
            type: "GET",
            // headers: {
    ...
    function searchData(category) {
        ...
        $("#quiz-table").find("tr:not(:first)").remove();
    ...
    function insertQuizdataToTable() {
        ...
        if (saveTableData == null || saveTableData.length < 1) {
            ...
            for (let i = 0; i < saveTableData.length; i++) {
                ...
                // searchDataTable.append(`
                // <tr>
                //     <td><img class='result-image' src='${saveTableData[i].image_location}' /></td>
                //     <td>
                //         <span style="padding-bottom: 10px; font-weight: bolder; font-size: xx-large;">${saveTableData[i].name}</span>
                //         <span>${saveTableData[i].description}</span>
                //     </td>
    ...
    function goToQuiz(quiz_id){
        let url = 'http://localhost:8082/CSCI201Quiz/quiz.html?quiz_id=' + quiz_id;
    ...
    function insertTradeEventData(tradeData, eventId) {
        ...
        //  };
        console.log('http://localhost:8082/CSCI201Quiz/favorites');

        $.ajax({
            url: 'http://localhost:8082/CSCI201Quiz/favorites?' + 'username=' + getCookie("username") + '&eventId=' + eventId,
            type: "GET",
            dataType: 'json',
            // data: JSON.stringify(sendData),
            // contentType: 'application/json',
    ...
    function updateFavoritedEvent(eventId, updateType) {
        $.ajax({
            url: 'http://localhost:8082/CSCI201Quiz/favorites?' + 'username=' + getCookie("username") + '&eventId=' + eventId + '&updateType=' + updateType,
            type: "POST",
            dataType: 'json',
            // data: JSON.stringify(sendData),
            // contentType: 'application/json',
    ...
    function doTradeEvent() {
        ...
        let ajaxUrl = 'http://localhost:8082/CSCI201Quiz/wallet?' + 'username=' + getCookie("username") + '&eventId=' + eventId + '&numTickets=' + quantity + '&price=' + tradePrice;
        console.log(ajaxUrl);
        $.ajax({
            type: "POST",
            // headers: {
